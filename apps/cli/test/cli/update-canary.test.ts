...
test("canary target version reads RYUZI_CANARY_TARGET", () => {
  expect(canaryTargetVersion("0.2.0", { RYUZI_CANARY_TARGET: "0.3.0", HARNESS_CANARY_TARGET: "0.4.0" })).toBe("0.3.0");
  expect(canaryTargetVersion("0.2.0", {})).toBe("0.2.0");
});

test("canary timeout reads RYUZI_CANARY_TIMEOUT_MS and ignores old harness env", () => {
  expect(canaryTimeoutMs({ RYUZI_CANARY_TIMEOUT_MS: "1234", HARNESS_CANARY_TIMEOUT_MS: "9999" })).toBe(1234);
  expect(canaryTimeoutMs({ HARNESS_CANARY_TIMEOUT_MS: "9999" })).toBe(60000);
