...
import type { Runner } from "../src/harness/detect";
...
const okRun: Runner = async (cmd) =>
  cmd[0] === "git"
    ? { exitCode: 0, stdout: "git version 2.45.0" }
    : { exitCode: 0, stdout: "2.1.89 (Claude Code)" };

const noClaude: Runner = async (cmd) =>
  cmd[0] === "git" ? { exitCode: 0, stdout: "git version 2.45.0" } : { exitCode: 127, stdout: "" };
