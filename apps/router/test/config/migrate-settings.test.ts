...
const set = (d: Database, k: string, v: string) => d.run("INSERT INTO settings(key,value) VALUES(?,?) ON CONFLICT(key) DO UPDATE SET value=excluded.value", [k, v]);

test("maps flat discord keys to namespaced and removes old", () => {
  const d = db();
  set(d, "discord_token", "tok"); set(d, "discord_app_id", "app"); set(d, "discord_guild_id", "gid");
  migrateSettings(d);
