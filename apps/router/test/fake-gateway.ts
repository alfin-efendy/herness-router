...
export class FakeGateway implements Gateway {
  ...
  async createWorkspace(name: string): Promise<string> { this.calls.push(`createWorkspace:${name}`); return `ws-${name}`; }
  async createConversation(workspaceId: string, title: string): Promise<string> {
    this.calls.push(`createConversation:${workspaceId}:${title}`); return `conv-${++this.n}`;
  }
  async postStatus(t: Surface, text: string): Promise<MessageRef> {
    this.calls.push(`postStatus:${t.conversationId}:${text}`); return { surface: t, messageId: `m-${++this.n}` };
  }
  async editStatus(ref: MessageRef, text: string): Promise<void> { this.calls.push(`editStatus:${ref.messageId}:${text}`); }
  async postResult(t: Surface, chunks: string[]): Promise<void> { this.calls.push(`postResult:${t.conversationId}:${chunks.join("|")}`); }
  async postError(t: Surface, text: string): Promise<void> { this.calls.push(`postError:${t.conversationId}:${text}`); }
