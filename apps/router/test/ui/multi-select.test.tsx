...
test("renders checkboxes + descriptions; space toggles current row", async () => {
  ...
  const items = [{ id: "a", label: "Alpha", description: "first" }, { id: "b", label: "Beta", description: "second" }];
  ...
  stdin.write("\x1b[B"); await flush(); // down arrow
  stdin.write(" "); await flush();        // toggle Beta
