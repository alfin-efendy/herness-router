...
export class ConsoleTelemetry implements Telemetry {
  constructor(private sink: (line: string) => void = console.error, private now: () => number = Date.now) {}

  startSpan(name: string, attrs: Attrs = {}): Span {
    const start = this.now();
    const a: Attrs = { ...attrs };
    let error: string | undefined;
    const emit = () => this.sink(JSON.stringify({ kind: "span", name, attrs: a, durationMs: this.now() - start, ...(error ? { error } : {}) }));
    return {
